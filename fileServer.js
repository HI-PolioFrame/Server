...
import { oriUsers } from './src/components/domain/startProgram'; 
import { initializeData } from './src/components/domain/startProgram'; 
...
app.post('/login', (req, res) => {
    ...
    if (!userId && !email || !password) {
        return res.status(400).send('아이디 또는 이메일과 비밀번호를 입력해주세요.');
    }

    // userId 또는 email로 사용자 찾기
    let user = null;
    if (userId) {
        user = oriUsers.get(userId); // Map의 get 메서드 사용
    } else if (email) {
        user = [...oriUsers.values()].find(user => user.email === email);
    }
    

    // userId 또는 email에 해당하는 사용자가 없으면 오류 메시지 반환
    if (!user) {
        return res.status(404).send('해당 아이디 또는 이메일을 찾을 수 없습니다.');
    }
